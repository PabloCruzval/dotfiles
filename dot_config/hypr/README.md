...
- **🎨 Noctalia v5 Colors** - Border and shadow colors applied via `require("noctalia").apply_theme()`
- **🎨 Centralized Fallback Colors** - Nord palette in `modules/fallback_colors.lua`
...
- **📱 Noctalia v5 Shell** - Native C++ status bar, launcher, control center, and notifications
...
└── modules/
    ...
    ├── inputs.lua                  # Keyboard, mouse, touchpad, gestures
    ├── decorations.lua             # Borders, shadows, blur, rounding
    ├── animations.lua              # Animation settings (Diablo-1 preset)
    ├── keybindings.lua             # All keybindings
    ├── windowrules.lua             # Window-specific rules
    ├── autostart.lua               # Startup applications
    ├── monitors-desktop.lua        # Desktop monitor setup
    └── monitors-laptop.lua         # Laptop monitor setup
```
...
**Color Scheme**: Dynamically generated by Noctalia v5 from the active wallpaper via `require("noctalia").apply_theme()`.

**Fallback Colors**: `modules/fallback_colors.lua` defines a static Nord palette used when Noctalia has not yet loaded or is unavailable. Hyprland border and shadow colors come from this file, so color values are centralized in one place instead of being hardcoded across modules.

| Element | Fallback Color | Variable |
|---------|----------------|----------|
| Active border | `#88c0d0` (Nord frost) | `colors.active_border` |
| Inactive border | `#3b4252` (Nord dark) | `colors.inactive_border` |
| Shadow | `#2e3440` (Nord darkest) | `colors.shadow` |

**Layout**:
- No gaps between windows (maximalist space usage)
- 15px corner rounding
- 2px borders for clear focus indication
- Left vertical bar reservation for Noctalia v5
...
Auto-started on login ([modules/autostart.lua](modules/autostart.lua)):

- **Noctalia v5** - Desktop shell (bars, launcher, control center)
- **Syncthing** - File synchronization
- **GNOME Keyring** - Credential storage

**GTK theming** is also enforced at startup via `gsettings`:
...
# Noctalia v5 app theming (handled by built-in templates)
```
...
- **[Noctalia v5](https://docs.noctalia.dev/v5/)** - Desktop shell and theming
- **[GTK](../gtk-3.0/README.md)** - GTK3/4 application theming
